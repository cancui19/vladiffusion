...
class LlavaLLaDAModelLM(LLaDAModelLM, LlavaMetaForCausalLM):
    ...
    @torch.no_grad()
    def generate_fix_mask(
        self,
        template_ids: Optional[torch.Tensor],
        inputs: Optional[torch.Tensor] = None,
        images: Optional[torch.Tensor] = None,
        image_sizes: Optional[torch.Tensor] = None,
        modalities: Optional[List[str]] = ["image"],
        **kwargs,
        ...
        return super().generate_with_embeds_fix_mask(prompt_embeds=inputs_embeds, template_ids=template_ids, **kwargs)
