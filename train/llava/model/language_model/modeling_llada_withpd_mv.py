...
class LLaDAModelLM(LLaDAPreTrainedModel):
    ...
    def generate_with_embeds(self, inputs_embeds, steps=128, gen_length=128, block_length=128, temperature=0.,
        cfg_scale=0., remasking='low_confidence', mask_id=126336, tokenizer=None, stopping_criteria=None, generation_suffix=None, confidence_threshold=0.9, **kwargs):
        '''
        Args:
            ...
            for num_block in range(num_blocks):
                ...
                for i in range(steps):
                    ...
                    if remasking == 'confidence_threshold':
                        ...
                        is_highest.view(-1)[highest_idx] = True
                        transfer_index = is_confident | is_highest
                        # k = mask_index.sum()
                        # transfer_index = torch.zeros_like(x0, dtype=torch.bool, device=x0.device)
                        # if k > 0:
