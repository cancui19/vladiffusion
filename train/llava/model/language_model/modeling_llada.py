...
class LLaDAModelLM(LLaDAPreTrainedModel):
        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            for num_block in range(num_blocks):
                ...
                for i in range(steps):
                    ...
                    # print(f"Check time: {check_time:.4f} seconds")
                    ...
                    if cfg_scale > 0.:
                        ...
                        # Forward pass
                        outputs = self.model(iautonputs_embeds=combined_embeds)
                        ...
                        # logits_time_start = time.time()
                        # Forward pass
                        # start_logits_event = torch.cuda.Event(enable_timing=True)
                        # end_logits_event = torch.cuda.Event(enable_timing=True)
                        # start_logits_event.record()
                        outputs = self.model(inputs_embeds=x_embeds)
                        logits = self.lm_head(outputs[0]).float()
                        # end_logits_event.record()
                        # torch.cuda.synchronize()
                        # logits_time = start_logits_event.elapsed_time(end_logits_event)
                        # print(f"Logits time: {logits_time:.4f} seconds")
                        # logits_time_end = time.time()
                    ...
                    if remasking == 'low_confidence' or remasking == 'confidence_threshold':
                    ...
                    confidence = torch.where(mask_index, x0_p, -np.inf)
                    # start_topk_event = torch.cuda.Event(enable_timing=True)
                    # end_topk_event = torch.cuda.Event(enable_timing=True)
                    # start_sum_event = torch.cuda.Event(enable_timing=True)
                    # end_sum_event = torch.cuda.Event(enable_timing=True)

                    if remasking == 'confidence_threshold':
                        # transfer_time_start = time.time()           
                        # print(f"max_confidence: {confidence.max()}")
                        # is_confident_start = time.time()
                        is_confident = confidence >= confidence_threshold
                        # is_confident_end = time.time()
                        # is_confident_time = is_confident_end - is_confident_start
                        # print(f"Is confident time: {is_confident_time:.4f} seconds")
                        transfer_index = is_confident
                        # highest_idx = torch.argmax(confidence)
                        # is_highest.view(-1)[highest_index] = True
                        # sum_start = time.time()
                        # start_sum_event.record()
                        if is_confident.sum() < num_transfer_tokens[0, i]:
                            # end_sum_event.record()
                            # torch.cuda.synchronize()
                            # sum_time = start_sum_event.elapsed_time(end_sum_event)
                            # print(f"Sum time: {sum_time:.4f} seconds")
                            # is_highest_start = time.time()
                            is_highest = torch.zeros_like(confidence, dtype=torch.bool)
                            # is_highest_end = time.time()
                            for j in range(confidence.shape[0]):
                                # topk_start = time.time()
                                # start_topk_event.record()
                                _, select_index = torch.topk(confidence[j], k=num_transfer_tokens[j, i])
                                # end_topk_event.record()
                                # torch.cuda.synchronize()
                                # topk_time = start_topk_event.elapsed_time(end_topk_event)
                                # print(f"Topk time: {topk_time:.4f} seconds")
                                # topk_end = time.time()
                                # topk_time = topk_end - topk_start
                                # print(f"Topk time: {topk_time:.4f} seconds")
                                is_highest[j, select_index] = True
                            # is_highest_time = is_highest_end - is_highest_start
                            # print(f"Is highest time: {is_highest_time:.4f} seconds")
                            transfer_index = is_highest | is_confident
                        # k = mask_index.sum()
                        # transfer_index = torch.zeros_like(x0, dtype=torch.bool, device=x0.device)
                        # if k > 0:
                        #     _, select_index = torch.topk(confidence.flatten(), k=k)
                        #     transfer_index.view(-1)[select_index] = True
                        #     for idx in select_index[1:]:
                        #         if confidence.view(-1)[idx] < confidence_threshold: # If the confidence is less than the threshold, set the transfer index to False
                        #             transfer_index.view(-1)[idx] = False
                        # print(confidence.max(), transfer_index.sum())
                    elif remasking == 'low_confidence' or remasking == 'random':
                        transfer_index = torch.zeros_like(x0, dtype=torch.bool, device=x0.device)
                        for j in range(confidence.shape[0]):
                            # topk_start = time.time()
                            _, select_index = torch.topk(confidence[j], k=num_transfer_tokens[j, i])
                    ...
                    # torch.cuda.synchronize()
                    # total_time = total_time_start_event.elapsed_time(total_time_end_event)
                    # print(f"Total time: {total_time:.4f} seconds")
                    if verbose and tokenizer is not None:
                        print(f"Block {num_block + 1}/{num_blocks}, Step {i + 1}/{steps}:")
                        # print(f"Generation time: {generation_time:.4f} seconds")
                        # Decode only the generated part of the sequence for clarity
                        generated_part = x[0, inputs_embeds.shape[1]:]
                        # Use skip_special_tokens=False to see all tokens, including stop tokens
                        decoded_text = tokenizer.decode(generated_part, skip_special_tokens=False)
                        
                        # The mask token might not have a clean string representation, so we find and replace it
                        mask_token_string = tokenizer.decode([mask_id])
                        if mask_token_string: # Avoid replacing if it decodes to an empty string
                            decoded_text = decoded_text.replace(mask_token_string, '*')
    ...
    @torch.no_grad()
    def generate_with_embeds_fix_mask(self, prompt_embeds, template_ids, steps=128, temperature=0.,
        cfg_scale=0., remasking='low_confidence', mask_id=126336, tokenizer=None, confidence_threshold=0.9, verbose=False, **kwargs):
        """
        Performs in-filling on a template based on a multimodal prompt.
 
        Args:
            prompt_embeds: Embeddings for the prompt part (text + images). Shape (1, P, D).
            template_ids: IDs for the template to be filled. Placeholders should use `mask_id`. Shape (1, T).
        ...
        with torch.cuda.amp.autocast(enabled=True):
            ...
            # 1. Prepare template state
            masked_embed = self.model.embed_tokens(torch.tensor([mask_id], device=device))
            
            template_ids_for_embedding = template_ids.clone()
            template_mask_bool = (template_ids == mask_id)
            # Clamp to avoid embedding invalid IDs like mask_id
            template_ids_for_embedding[template_mask_bool] = 0
            template_embeds = self.model.embed_tokens(template_ids_for_embedding)
            
            if template_mask_bool.any():
                num_masks = template_mask_bool.sum()
                template_embeds[template_mask_bool] = masked_embed.expand(num_masks, -1)
 
            # 2. Combine prompt and template into a single sequence for the model
            # Create placeholder IDs for the prompt part internally
            prompt_ids_internal = torch.full((1, prompt_len), -100, dtype=torch.long, device=device)
            x_ids = torch.cat([prompt_ids_internal, template_ids], dim=1)
            x_embeds = torch.cat([prompt_embeds, template_embeds], dim=1)
            
            # This mask is True only for the [MASK] tokens in the template part.
            infill_mask = torch.zeros_like(x_ids, dtype=torch.bool)
            infill_mask[:, prompt_len:] = template_mask_bool
 
            if not infill_mask.any():
                return template_ids
 
            # 3. Setup for iterative generation
            num_transfer_tokens = self.get_num_transfer_tokens(infill_mask, steps)
            prompt_index = (x_ids != mask_id)
            feature_cache = dLLMCache()
            feature_cache.reset_cache(prompt_embeds.shape[1])
            # 4. Main generation loop
            for i in range(steps):
                current_mask = (x_ids == mask_id)
                ...
                if cfg_scale > 0.:
                    ...
                    num_unmasked_tokens = un_mask.sum() // x_embeds.shape[-1]
                    un_embeds[un_mask] = masked_embed.expand(int(num_unmasked_tokens), -1)
                    combined_embeds = torch.cat([x_embeds, un_embeds], dim=0)
                ...
                transfer_index = torch.zeros_like(x_ids, dtype=torch.bool)
                if num_transfer_tokens[0, i] > 0:
                    if remasking == 'confidence_threshold':
                        # transfer_index = (confidence >= confidence_threshold)
                        is_confident = confidence >= confidence_threshold
                        transfer_index = is_confident
                        # highest_idx = torch.argmax(confidence)
                        # is_highest.view(-1)[highest_index] = True
                        ...
                        if num_to_transfer > 0:
                            ...
                            if k > 0:
                                _, select_indices = torch.topk(confidence.flatten(), k=k)
                                transfer_index.view(-1)[select_indices] = True
 
                x0_candidate = torch.where(current_mask, x0, x_ids)
                
                newly_predicted_ids = x0_candidate[transfer_index]
                if newly_predicted_ids.numel() > 0:
                    assert (newly_predicted_ids > 0).all()
                    newly_predicted_embeds = self.model.embed_tokens(newly_predicted_ids)
                    x_embeds[transfer_index] = newly_predicted_embeds
                    x_ids[transfer_index] = newly_predicted_ids
 
                if verbose and tokenizer is not None:
                    print(f"Step {i + 1}/{steps}:")
                    printable_x = x_ids[:, prompt_len:].clone()
                    decoded_text = tokenizer.decode(printable_x[0], skip_special_tokens=False)
                    mask_token_string = tokenizer.decode([mask_id])
                    if mask_token_string:
                        decoded_text = decoded_text.replace(mask_token_string, '*')
                    print(decoded_text)
                    print("-" * 80)
 
            # 5. Return the filled template part
            return x_ids[:, prompt_len:]
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if True:
                ...
                # shortcut loss
                with torch.no_grad():
                    true_indices_coords = masked_indices.nonzero(as_tuple=True)
                    num_masked = len(true_indices_coords[0])
                    remasked_logits = None
                    remask_indices = None

                    if num_masked > 0:
                        num_to_remask = num_masked // 2
                        if num_to_remask > 0:
                            perm = torch.randperm(num_masked, device=masked_indices.device)
                            selected_indices_in_masked = perm[:num_to_remask]
                            
                            remask_coords = (true_indices_coords[0][selected_indices_in_masked], 
                                            true_indices_coords[1][selected_indices_in_masked])

                            remask_indices = torch.zeros_like(masked_indices, dtype=torch.bool)
                            remask_indices[remask_coords] = True

                            x0 = torch.argmax(logits, dim=-1)
                            x0_embeds = self.model.embed_tokens(x0)

                            intermediate_embeds = torch.where(masked_indices.unsqueeze(-1), x0_embeds, noisy_embeds)
                            remasked_noisy_embeds = torch.where(remask_indices.unsqueeze(-1), masked_embed, intermediate_embeds)

                            remasked_outputs = self.model(
                                inputs_embeds=remasked_noisy_embeds,
                                attention_mask=attention_mask,
                                position_ids=position_ids,
                                past_key_values=past_key_values,
                                use_cache=use_cache,
                            ...
                            remasked_hidden_states = remasked_outputs[0]
                            remasked_logits = self.lm_head(remasked_hidden_states).float()
                
                if remasked_logits is not None and remask_indices is not None:
                    log_probs_orig = F.log_softmax(logits[remask_indices], dim=-1)
                    probs_new = F.softmax(remasked_logits[remask_indices], dim=-1)

                    shortcut_token_loss = -torch.sum(probs_new * log_probs_orig, dim=-1)
