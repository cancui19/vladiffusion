...
with open('/scratch/gautschi/mgagvani/new_nuscenes/downloads/nuscenes_waypoint_long_prompt_val.json', 'r') as f:
...
for i, data_sample in enumerate(data_val):
    ...
    #     continue
    print("Image path: ", data_sample['image'])
    ...
    ids_tensor = torch.from_numpy(np.load("tokenizer/unused_token_ids.npy")).to(device)
    ...
    print("Recovered Points: ", recovered_point)
    exit()
    ...
    with open(f'nuscenes_drive_data_single_image_val_inference_lora_{job_name}.json', 'w') as f:
...
with open(f'nuscenes_drive_data_single_image_val_inference_lora_{job_name}_time.txt', 'w') as f:
