...
job_name = 'single_image_train_lora'
...
# with open(f'/depot/ziran/apps/jiaru/projects/LLaDA-AV/results/nuscenes_drive_data_single_image_val_inference_lora_single_image_train_lora062525.json', 'r') as f:
#     inference_results = json.load(f)

vis_data_dir = "generation_visual_data"
...
for i, data_sample in enumerate(data_val):
    ...
    #     steps=128, gen_length=128, block_length=128, tokenizer=tokenizer, stopping_criteria=['<|eot_id|>'], verbose=True
    # )
    cont = model.generate_fix_mask(
        ...
        images=image_tensor,
        image_sizes=image_sizes,
        steps=32, tokenizer=tokenizer, stopping_criteria=['<|eot_id|>'], verbose=True,
    )
    end_time = time.time()
